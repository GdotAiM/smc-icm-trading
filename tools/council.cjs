...
console.log(JSON.stringify(councilOutput, null, 2));

// ── LLM Narrative (opt-in via --narrative flag) ────────────────────────────────
const NARRATIVE_MODE = process.argv.includes("--narrative");

if (NARRATIVE_MODE) {
  const narrate = async () => {
    // Gather context — read recent stage outputs for richer narrative
    let pairContext = "";
    try {
      const biasFile = path.join(ROOT, "stages", "01_htf_bias", "output", "bias.md");
      if (fs.existsSync(biasFile)) {
        pairContext += "### HTF Bias\n" + fs.readFileSync(biasFile, "utf8").slice(0, 800) + "\n\n";
      }
      const levelsFile = path.join(ROOT, "stages", "02_key_levels", "output", "levels.md");
      if (fs.existsSync(levelsFile)) {
        pairContext += "### Key Levels\n" + fs.readFileSync(levelsFile, "utf8").slice(0, 800) + "\n\n";
      }
    } catch (_) { /* best-effort context */ }

    try {
      const { chatCompletion } = require("./llm/llm_client.cjs");
      const { councilNarrative } = require("./llm/llm_prompts.cjs");
      const { messages, config } = councilNarrative(councilOutput, pairContext);

      console.error(`\n🧠 Generating LLM Narrative...\n`);
      const response = await chatCompletion(messages, config);

      if (response.text.startsWith("[LLM")) {
        console.error(`⚠️  LLM unavailable: ${response.text}`);
        console.error(`(Set GEMINI_API_KEY in .env for council narrative)`);
      } else {
        console.error(`${"=".repeat(70)}`);
        console.error(`  PRE-TRADE BRIEFING — ${pairLabel} — ${DATE}`);
        console.error(`${"=".repeat(70)}\n`);
        console.error(response.text);
        console.error(`\n${"─".repeat(70)}`);
        console.error(`Generated by ${response.provider}/${response.model}`);

        // Write narrative to council output directory
        const narrativeFile = path.join(outDir, `${PAIR.toLowerCase()}_narrative.md`);
        const narrativeMd = `# Pre-Trade Briefing — ${pairLabel} — ${DATE}\n\n` +
          `**Verdict**: ${verdict}  \n` +
          `**Confidence**: ${confidencePct}%  \n` +
          `**Generated by**: ${response.provider}/${response.model}\n\n` +
          `---\n\n` +
          response.text + `\n\n` +
          `---\n\n` +
          `*Run \`node tools/council.cjs ${PAIR} --narrative\` to regenerate.*\n`;
        fs.writeFileSync(narrativeFile, narrativeMd, "utf8");
        console.error(`\n  📄 Narrative saved: stages/00_council_vote/output/${PAIR.toLowerCase()}_narrative.md`);
      }
    } catch (e) {
      console.error(`⚠️  Narrative generation skipped: ${e.message}`);
    }
  };
  narrate();
}
