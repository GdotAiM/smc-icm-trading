# IPDA Dealing Range Analysis — EURUSD — 2026-08-02

## Nested Dealing Ranges (Macro → Micro)

| Timeframe | IPDA20 EQ | IPDA40 EQ | IPDA60 EQ | Zone | Position |
|-----------|-----------|-----------|-----------|------|----------|
| 1W | 1.15869 | — | — | DISCOUNT (buy) | 8.77% |
| 1D | 1.14221 | 1.14947 | 1.15606 | DISCOUNT (buy) | 7.30% |
| 4H | 1.13998 | 1.14201 | — | DISCOUNT (buy) | 9.44% |
| 1H | 1.13831 | 1.13994 | — | DISCOUNT (buy) | 15.47% |
| 15m | 1.13769 | — | — | DISCOUNT (buy) | 16.84% |

## Equilibrium Cascade (Stepping Stones)

Price delivers from one equilibrium to another. Each level acts as a checkpoint.

```
1W   EQ 1.15869      DISCOUNT (buy)     █████████ 8.77%
1D   EQ 1.14221      DISCOUNT (buy)     ██ 7.30%
4H   EQ 1.13998      DISCOUNT (buy)     █ 9.44%
1H   EQ 1.13831      DISCOUNT (buy)     █ 15.47%
15m  EQ 1.13769      DISCOUNT (buy)      16.84%
```

## IPDA Draw Direction

**UP (toward equilibrium)**
Price is in DISCOUNT across macro (1W) and micro (15m). IPDA is drawing price UP toward equilibrium. Buy-side delivery domain.

**Zone Consensus**: DISCOUNT (0/5 premium, 5/5 discount) — STRONG consensus

## AMD on the Dealing Range

**ACCUMULATION ZONE — Price at range extreme. Institutions building positions.**
Daily range: DISCOUNT (buy) at 13.52% of range. ACCUMULATION ZONE — Price at range extreme. Institutions building positions. EQ @ 1.14947.

## Quarterly Shift

**Quarter month 2. Standard IPDA behavior expected.**

## How the IPDA Lens Changes Your Trade

- **Macro (1W)**: Price is DISCOUNT (buy) — institutional buy zone. Look for LONGS only when LTF confirms.
- **Meso (1D)**: Price is DISCOUNT (buy) at 7.30% of range.
- **Micro (4H)**: Price is DISCOUNT (buy). ✅ ALIGNED with macro — trade with confidence.

- **Equilibrium Gravity**: The 1W equilibrium at 1.15869 is the macro fair value. Price is -1.87% below it. The algorithm will seek to return to this level.

- **Cascading Delivery**: Price must pass through each equilibrium checkpoint: 1W @ 1.15869 → 1D @ 1.14221 → 4H @ 1.13998 → 1H @ 1.13831 → 15m @ 1.13769. Each is a potential reaction zone.

---

*"The IPDA doesn't move randomly. It delivers price from one dealing range extreme to the other, hunting liquidity at every equilibrium checkpoint along the way."*

## False Breakout Detection
**BULL TRAP (20-day HIGH swept → reversal DOWN)**
⚠️ FALSE BREAKOUT: 20-day high @ 1.14824 swept, price reversed back inside. IPDA stop-hunt before bearish reversal. Target: 40-day low @ 1.13246 / 60-day low @ 1.13246.
- Direction: BEARISH
- Target: 40-day @ 1.13246 / 60-day @ 1.13246

## Kill Zone Alignment
✅ NY AM KZ active (weight: 1.3) — HIGH conviction for IPDA reversals.
- Active Zone: NY AM KZ (weight: 1.3)

## IPDA Objective: REBALANCE (post-hunt)
Liquidity swept (8 pools). 1 unfilled FVGs remain — IPDA now rebalancing imbalances. Price drawing toward unfilled FVGs.
- Unfilled FVGs: 1 | Swept Pools: 8 | Unswept: 4

## Weekly Reference Levels (Marked 2026-08-02)
20-Day: H 1.14824 L 1.13618 EQ 1.14221 | 40-Day: H 1.16648 L 1.13246 EQ 1.14947 | 60-Day: H 1.17967 L 1.13246 EQ 1.15606
