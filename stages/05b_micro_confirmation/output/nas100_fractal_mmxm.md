# Fractal MMXM Coherence — NAS100 — 2026-08-05

## HTF Direction: **BULLISH**

## MMXM Steps Per Timeframe

| TF | Step | Label | Confidence | Narrative |
|----|------|-------|------------|-----------|
| 1W | **2** | MANIPULATION | 0.85 | Sweep (BSL,SSL) + CHoCH reversal. Classic manipulation. The trap is set — wait for reversal to confirm. |
| 1D | **2** | MANIPULATION | 0.85 | Sweep (SSL,SSL,SSL) + CHoCH reversal. Classic manipulation. The trap is set — wait for reversal to confirm. |
| 4H | **2** | MANIPULATION | 0.85 | Sweep (SSL,SSL,BSL) + CHoCH reversal. Classic manipulation. The trap is set — wait for reversal to confirm. |
| 1H | **3** | DISTRIBUTION | 0.80 | BOS bullish confirmed. 2 FVG(s). Displacement: moderate (1.00x). Trend is distributing. |
| 15m | **2** | MANIPULATION | 0.85 | Sweep (SSL,SSL,SSL) + CHoCH reversal. Classic manipulation. The trap is set — wait for reversal to confirm. |
| 5m | **3** | DISTRIBUTION | 0.60 | BOS bullish detected. Distribution beginning. Waiting for displacement confirmation. |
| 1m | **2** | MANIPULATION | 0.85 | Sweep (BSL,SSL) + CHoCH reversal. Classic manipulation. The trap is set — wait for reversal to confirm. |

## Fractal Nesting Check (6 pairs)

- ✅ 1W Step 2 → 1D Step 2 — 1W Step 2 → 1D Step 2 — correctly nested
- ✅ 1D Step 2 → 4H Step 2 — 1D Step 2 → 4H Step 2 — correctly nested
- ✅ 4H Step 2 → 1H Step 3 — 4H Step 2 → 1H Step 3 — correctly nested
- ❌ 1H Step 3 → 15m Step 2 — 1H Step 3 → 15m Step 2 — LTF behind HTF. Nesting broken.
- ✅ 15m Step 2 → 5m Step 3 — 15m Step 2 → 5m Step 3 — correctly nested
- ❌ 5m Step 3 → 1m Step 2 — 5m Step 3 → 1m Step 2 — LTF behind HTF. Nesting broken.

**Nesting Score**: 4/6 — ⚠️ 2 nesting break(s). Minor misalignment — the fractal is mostly intact.

## 1m Inversion Detection

**Score**: 4/8

| Signal | Status |
|--------|--------|
| CHoCH on 1m | ✅ |
| Recent sweep on 1m | ✅ |
| 1m bias aligned with HTF (bullish) | ✗ |
| Entry FVG on 1m | ✗ |
| Strong displacement | ✗ |

**⏳ 1m Inversion BUILDING — 4/8 signals. CHoCH ✓. Sweep ✓.**

## 5m CISD (Change in State of Delivery)

**Insufficient candle data**

## SMT Divergence

**No SMT divergence detected. Check correlated pairs manually.**

## The 6 Confirmations

| # | Confirmation | Status | Detail |
|---|-------------|--------|--------|
| 1 | SMT Divergence | ✗ | No SMT divergence detected. Check correlated pairs manually. |
| 2 | Liquidity Sweep | ✅ | Sweep detected on 1H |
| 3 | MSS / CHoCH | ✅ | 5m event: BOS |
| 4 | CISD (Engulfing) | ✗ | Insufficient candle data |
| 5 | FVG Creation | ✗ | 5m: 0 FVGs, 1m: 0 FVGs |
| 6 | HTF PD Array | ✅ | HTF at Step 2 — PD Array context active |

**Passed**: 3/6

## Fractal Coherence Score: **11/20**

**⏳ MODERATE fractal coherence — some nesting gaps. Wait for tighter alignment.**

---

*"The MMXM on the 4H is the STORY. The 5m is the CHAPTER. The 1m is the SENTENCE where you enter."*
